...
var serviceDescription = &winservice.ServiceDescription{
	Name:        "pugdagdsvc",
	DisplayName: "Pugdagd Service",
	Description: "Downloads and stays synchronized with the Kaspa blockDAG and " +
		"provides DAG services to applications.",
}

type pugdagdApp struct {
	cfg *config.Config
}

// StartApp starts the pugdagd app, and blocks until it finishes running
func StartApp() error {
...
func (app *pugdagdApp) main(startedChan chan<- struct{}) error {
	...
	if err != nil {
		log.Errorf("Unable to start pugdagd: %+v", err)
		return err
	}

	defer func() {
		log.Infof("Gracefully shutting down pugdagd...")
		...
		log.Infof("Pugdagd shutdown complete")
	}()
