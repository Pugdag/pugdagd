...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		pugdagdMessage, err := client.Post(&protowire.PugdagdMessage{Payload: &protowire.PugdagdMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := pugdagdMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	pugdagdMessage := &protowire.PugdagdMessage{}
	err := protojson.Unmarshal([]byte(response), pugdagdMessage)
	if err != nil {
	...
	return marshalOptions.Format(pugdagdMessage)
}
