...
// 2. A wrapper that implements isPugdagdMessage_Payload, having a single field pointing to the concrete command
//    bearing the name of the command with `PugdagdMessage_` prefix and `Request` suffix
...
// generatePugdagdMessage generates a wrapped PugdagdMessage with the given `commandValue`
func generatePugdagdMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.PugdagdMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	pugdagdMessage := reflect.New(reflect.TypeOf(protowire.PugdagdMessage{}))
	pugdagdMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return pugdagdMessage.Interface().(*protowire.PugdagdMessage), nil
}
