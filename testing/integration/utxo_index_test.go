...
func TestUTXOIndex(t *testing.T) {
	// Setup a single pugdagd instance
	...
	err := pugdagd.rpcClient.RegisterForUTXOsChangedNotifications([]string{miningAddress1}, func(
	...
	getCoinSupplyResponse, err := pugdagd.rpcClient.GetCoinSupply()
	if err != nil {
		t.Fatalf("Error Retriving Coin supply: %s", err)
	}

	rewardsMinedSompi := uint64(blockAmountToMine * constants.SompiPerKaspa * 500)
	getBlockCountResponse, err := pugdagd.rpcClient.GetBlockCount()
	if err != nil {
	...
	for i := 0; i < transactionAmountToSpend; i++ {
		...
		_, err = pugdagd.rpcClient.SubmitTransaction(rpcTransaction, false)
		if err != nil {
	...
	utxosByAddressesResponse, err := pugdagd.rpcClient.GetUTXOsByAddresses([]string{miningAddress1})
	if err != nil {
