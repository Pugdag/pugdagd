...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of pugdagd instances
	pugdagd1, pugdagd2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := pugdagd1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In pugdagd2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, pugdagd2)
	}

	// Connect the two pugdagds. This should trigger sync
	// between the two nodes
	connect(t, pugdagd1, pugdagd2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := pugdagd1.rpcClient.GetVirtualSelectedParentChainFromBlock(
