...
// long-running pugdagdMain (which is the real meat of pugdagd), handles service
...
func (s *Service) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	...
	// Start pugdagdMain in a separate goroutine so the service can start
	// quickly. Shutdown (along with a potential error) is reported via
	// doneChan. startedChan is notified once pugdagd is started so this can
	// be properly logged
	doneChan := make(chan error)
	startedChan := make(chan struct{})
	spawn("pugdagdMain-windows", func() {
...
// logServiceStart logs information about pugdagd when the main server has
