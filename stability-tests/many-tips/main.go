...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("pugdagd-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("pugdagd datadir: %s", dataDir)

	pugdagdCmd, err := common.StartCmd("KASPAD",
		"pugdagd",
	...
	spawn("startNode-pugdagdCmd.Wait", func() {
		err := pugdagdCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("pugdagdCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes pugdagd closes ungracefully
				log.Errorf("pugdagdCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(pugdagdCmd, "pugdagdCmd")
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	pugdagminerCmd, err := common.StartCmd("MINER",
		"pugdagminer",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
		"-s", rpcAddress,
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(pugdagminerCmd, "pugdagminerCmd")
	return nil
}
